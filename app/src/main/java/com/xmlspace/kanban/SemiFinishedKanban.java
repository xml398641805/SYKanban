...
public class SemiFinishedKanban extends AppCompatActivity
        implements NavigationView.OnNavigationItemSelectedListener {

    //region 看板列表属性
    //ListView中显示的数据项列表
    private List<SemiFinishedItem> semiFinishedItemList;
    private SemiFinishedListAdapter semiFinishedListAdapter;
    //ListView显示分页信息
    private PagesInfo semifinishedPageInfo;
    //当前显示页数
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_semi_finished_kanban);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.semi_finished_kanban, menu);
        return true;
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        int id = item.getItemId();
        if(id==R.id.nav_semi_finished_config){
            ShowPreferenceAlert();
        }else if(id==R.id.nav_semi_finished_exit){
    ...
    //显示看板数据，并设置列表高度
    private void initListHigh(List<SemiFinishedItem> semiFinishedItems){
        semiFinishedListAdapter=new SemiFinishedListAdapter(SemiFinishedKanban.this,semiFinishedItems);
        ListView lv=(ListView)findViewById(R.id.semi_finished_list_item);
        lv.setAdapter(semiFinishedListAdapter);
    ...
    private boolean initPageInfo(){
        ...
        try{
            ...
            kanbanValue=WebServiceUtil2.getSemiFinishedPageInfo((double)showLineNum);
            if(kanbanValue.errorSign||kanbanValue.semiFinishedPagesInfo==null) {
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        TextView tv_page = (TextView) findViewById(R.id.tv_page);
                        tv_page.setText("分页信息错误，请等待下次更新！");
                        Toast.makeText(SemiFinishedKanban.this, kanbanValue.errorMessage, Toast.LENGTH_LONG).show();
                    }
                });
                return false;
            }
            semifinishedPageInfo=kanbanValue.semiFinishedPagesInfo;
            currentShowPage=1;
            this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    TextView tv_page = (TextView) findViewById(R.id.tv_page);
                    tv_page.setText("总页数："+String.valueOf((int)semifinishedPageInfo.TotalPages)+"，当前第1页！");
                }
            });
            kanbanValue=null;
        }catch(Exception ex){
            this.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    Toast.makeText(SemiFinishedKanban.this, "启动初始化分页信息失败", Toast.LENGTH_SHORT).show();
                }
            });
            return false;
        }
    ...
    private boolean initShowItemList(){
        try{
            ...
            switch(currentWorkType){
                case AllShow:
                    kanbanValue=WebServiceUtil2.getSemiFinishedItems(WebServiceUtil2.ShowType.ShowAll,0.0,0.0,"");
                    break;
                case NextShow:
                    if(currentShowPage>=semifinishedPageInfo.TotalPages){
                        currentShowPage=1;
                        currentWorkType= workType.InitShow;
                    }else{
                        currentShowPage+=1;
                    }
                    kanbanValue=WebServiceUtil2.getSemiFinishedItems(WebServiceUtil2.ShowType.NextPage,(double)currentShowPage,showRecordNum,"");
                    break;
                case PreviousShow:
                    if(currentShowPage==1){
                        currentShowPage=(int)semifinishedPageInfo.TotalPages;
                        currentWorkType= workType.InitShow;
                    }else {
                        currentShowPage-=1;
                    }
                    kanbanValue=WebServiceUtil2.getSemiFinishedItems(WebServiceUtil2.ShowType.PreviousPage,(double)currentShowPage,(double)myPreference.getListShowRecordNum(),"");
                    break;
                case InitShow:
                default:
                    kanbanValue=WebServiceUtil2.getSemiFinishedItems(WebServiceUtil2.ShowType.InitShow,(double)currentShowPage,showRecordNum,"");
                    break;
            }
            if(kanbanValue.errorSign||kanbanValue.semiFinishedItems.size()==0){
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(SemiFinishedKanban.this, "看板当前不存在可用数据！", Toast.LENGTH_SHORT).show();
                        TextView tv_page = (TextView) findViewById(R.id.tv_page);
                ...
                semiFinishedItemList=kanbanValue.semiFinishedItems;
                kanbanValue=null;
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        //显示看板数据，并设置列表高度
                        initListHigh(semiFinishedItemList);
                        TextView tv_page = (TextView) findViewById(R.id.tv_page);
                        if(currentWorkType== workType.AllShow){
                            tv_page.setText("当前显示全部数据记录！");
                        }else {
                            tv_page.setText("总页数："+String.valueOf((int)semifinishedPageInfo.TotalPages)+"，当前第"+String.valueOf(currentShowPage)+"页！");
                        }
                    }
                });
            }
            ...
            this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(SemiFinishedKanban.this, "启动显示数据记录失败", Toast.LENGTH_SHORT).show();
                    TextView tv_page = (TextView) findViewById(R.id.tv_page);
                    tv_page.setText("看板当前不存在可用数据，请等待下次更新！");
                }
            });
    ...
    private boolean initShowWhereItemList(String Where){
        try{
            kanbanValue=WebServiceUtil2.getSemiFinishedItems(WebServiceUtil2.ShowType.Where,0.0,0.0,Where);
            if(kanbanValue.errorSign||kanbanValue.semiFinishedItems.size()==0){
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(SemiFinishedKanban.this, "在指定筛选条件下，看板当前不存在可用数据！", Toast.LENGTH_SHORT).show();
                    }
                });
                return false;
            }else {
            ...
            this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(SemiFinishedKanban.this, "启动筛选显示数据记录失败", Toast.LENGTH_SHORT).show();
                }
            });
            return false;
        }
