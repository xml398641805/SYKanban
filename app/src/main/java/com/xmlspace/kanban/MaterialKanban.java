...
public class MaterialKanban extends AppCompatActivity
    ...
    //ListView显示分页信息
    private PagesInfo materialPageInfo;
    //当前显示页数
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_material_kanban);
        Toolbar toolbar = (Toolbar) findViewById(R.id.material_kanban_toolbar);
        toolbar.setTitle("");
        setSupportActionBar(toolbar);

        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.material_kanban_drawer_layout);
        ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(
                this, drawer, toolbar, R.string.navigation_drawer_open, R.string.navigation_drawer_close);
        drawer.addDrawerListener(toggle);
        toggle.syncState();

        NavigationView navigationView = (NavigationView) findViewById(R.id.material_kanban_nav_view);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.material_kanban, menu);
        return true;
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        int id = item.getItemId();
        try {
           if (id == R.id.material_kanban_nav_settings) {
                //Toast.makeText(this, "系统配置", Toast.LENGTH_SHORT).show();
                ShowPreferenceAlert();
            } else if (id == R.id.material_kanban_nav_exit) {
            ...
            Toast.makeText(this, "功能执行错误，返回错误信息："+ex.toString(), Toast.LENGTH_LONG).show();
        }
        return true;
    }

    //显示看板数据，并设置列表高度
    private void initListHigh(List<MaterialItem> materialItems){
        materialListAdapter=new MaterialListAdapter(MaterialKanban.this,materialItems);
        ListView lv=(ListView)findViewById(R.id.material_list_item);
        lv.setAdapter(materialListAdapter);
    ...
    private boolean initPageInfo(){
        ...
        try{
            ...
            kanbanValue=WebServiceUtil2.getMaterialPageInfo((double)showLineNum);
            if(kanbanValue.errorSign||kanbanValue.materialPagesInfo==null) {
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        TextView tv_page = (TextView) findViewById(R.id.tv_page);
                        tv_page.setText("分页信息错误，请等待下次更新！");
                        Toast.makeText(MaterialKanban.this, kanbanValue.errorMessage, Toast.LENGTH_SHORT).show();
                    }
                });
                return false;
            }
            materialPageInfo=kanbanValue.materialPagesInfo;
            currentShowPage=1;
            this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    TextView tv_page = (TextView) findViewById(R.id.tv_page);
                    tv_page.setText("总页数："+String.valueOf((int)materialPageInfo.TotalPages)+"，当前第1页！");
                }
            });
            kanbanValue=null;
        }catch(Exception ex){
            this.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    Toast.makeText(MaterialKanban.this, "启动初始化分页信息失败", Toast.LENGTH_SHORT).show();
                }
            });
            return false;
        }
    ...
    private boolean initShowItemList(){
        try{
            ...
            switch(currentWorkType){
                case AllShow:
                    kanbanValue=WebServiceUtil2.getMaterialItems(WebServiceUtil2.ShowType.ShowAll,0.0,0.0,"");
                    break;
                case NextShow:
                    if(currentShowPage>=materialPageInfo.TotalPages){
                        currentShowPage=1;
                        currentWorkType=workType.InitShow;
                    }else{
                        currentShowPage+=1;
                    }
                    kanbanValue=WebServiceUtil2.getMaterialItems(WebServiceUtil2.ShowType.NextPage,(double)currentShowPage,showRecordNum,"");
                    break;
                case PreviousShow:
                    if(currentShowPage==1){
                        currentShowPage=(int)materialPageInfo.TotalPages;
                        currentWorkType=workType.InitShow;
                    }else {
                        currentShowPage-=1;
                    }
                    kanbanValue=WebServiceUtil2.getMaterialItems(WebServiceUtil2.ShowType.PreviousPage,(double)currentShowPage,(double)myPreference.getListShowRecordNum(),"");
                    break;
                case InitShow:
                default:
                    kanbanValue=WebServiceUtil2.getMaterialItems(WebServiceUtil2.ShowType.InitShow,(double)currentShowPage,showRecordNum,"");
                    break;
            }
            if(kanbanValue.errorSign||kanbanValue.materialItems.size()==0){
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(MaterialKanban.this, "看板当前不存在可用数据！", Toast.LENGTH_SHORT).show();
                        TextView tv_page = (TextView) findViewById(R.id.tv_page);
                ...
                materialItemList=kanbanValue.materialItems;
                kanbanValue=null;
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        //显示看板数据，并设置列表高度
                        initListHigh(materialItemList);
                        TextView tv_page = (TextView) findViewById(R.id.tv_page);
                        if(currentWorkType==workType.AllShow){
                            tv_page.setText("当前显示全部数据记录！");
                        }else {
                            tv_page.setText("总页数："+String.valueOf((int)materialPageInfo.TotalPages)+"，当前第"+String.valueOf(currentShowPage)+"页！");
                        }
                    }
                });
            }
            ...
            this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(MaterialKanban.this, "启动显示数据记录失败", Toast.LENGTH_SHORT).show();
                    TextView tv_page = (TextView) findViewById(R.id.tv_page);
                    tv_page.setText("看板当前不存在可用数据，请等待下次更新！");
                }
            });
    ...
    private void initShowLessThanMin(){
        Thread thread=new Thread(new Runnable() {
            @Override
            public void run() {
                if(initShowWhereItemList(" Where MaterialStock<MinStock")){
    ...
    private void initShowHighThanMax(){
        Thread thread=new Thread(new Runnable() {
            @Override
            public void run() {
                if(initShowWhereItemList(" Where MaterialStock>MaxStock")){
                    currentWorkType=workType.WaitShow;
                    currentWaitProcess=WaitProcess.Finished;
                    lastOperateTime=new Date();
                }
    ...
    private void initShowFinishedDelivery(){
        Thread thread=new Thread(new Runnable() {
            @Override
            public void run() {
                if(initShowWhereItemList(" Where WaitDelivery=0")){
                    currentWorkType=workType.WaitShow;
                    currentWaitProcess=WaitProcess.Finished;
                    lastOperateTime=new Date();
                }
    ...
    private boolean initShowWhereItemList(String Where){
        try{
            kanbanValue=WebServiceUtil2.getMaterialItems(WebServiceUtil2.ShowType.Where,0.0,0.0,Where);
            if(kanbanValue.errorSign||kanbanValue.materialItems.size()==0){
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(MaterialKanban.this, "在指定筛选条件下，看板当前不存在可用数据！", Toast.LENGTH_SHORT).show();
                    }
                });
                return false;
            }else {
            ...
            this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(MaterialKanban.this, "启动筛选显示数据记录失败", Toast.LENGTH_SHORT).show();
                }
            });
            return false;
        }
